...
=== [[CPD-run]]start.sh
The script bundles the CPD configuration and run commands. First, the run script invokes the configuration script (<<configure, configure.sh>>). 
...
Explicit re-configurations of such parameters must be invoked through the (<<configure, configure.sh>>) script.
