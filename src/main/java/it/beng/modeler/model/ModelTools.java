...
public class ModelTools {
    ...
    public void getEntity(String id, String collection, Handler<AsyncResult<JsonObject>> handler) {
        ...
        mongodb.findOne(collection, query, new JsonObject(), JSON_ENTITY_TO_MONGO_DB, findOne -> {
        ...
        schemaTools.validate(schemaTools.absRef(entity.getString("class")), entity, validate -> {
            if (validate.succeeded()) {
                if (validate.result().isValid()) {
                    mongodb.save(collection, entity, JSON_ENTITY_TO_MONGO_DB, save -> {
                        if (save.succeeded()) {
                            handler.handle(Future.succeededFuture(
                                new SaveResult<>(validate.result(), save.result())));
                        } else handler.handle(Future.failedFuture(save.cause()));
                    });
                } else handler.handle(Future.succeededFuture(new SaveResult<>(validate.result(), entity)));
            } else handler.handle(Future.failedFuture(validate.cause()));
        });
    }

    public void deleteEntity(String id, String collection, Handler<AsyncResult<DeleteResult>> handler) {
        mongodb.removeDocument(collection, new JsonObject().put("id", id), JSON_ENTITY_TO_MONGO_DB,
            removeDocument -> {
    ...
    public void getDiagramElements(String rootId, Handler<AsyncResult<List<JsonObject>>> handler) {
        JsonObject query = new JsonObject()
            .put("$or", new JsonArray()
                .add(new JsonObject().put("id", rootId))
                .add(new JsonObject().put("rootId", rootId)));
        mongodb.find("diagram", query, JSON_ENTITY_TO_MONGO_DB, find -> {
            if (find.succeeded()) {
    ...
    public void getDiagramModels(String rootId, Handler<AsyncResult<JsonArray>> handler) {
        ...
        mongodb.runCommand("aggregate", command, JSON_ENTITY_TO_MONGO_DB, runCommand -> {
